...
## Lendo os dados ----
df  <- fread(paste0(wddata, "EnvQuenching/assign/", input_file)) 

## Corrigindo MANGLE ----
mangle_L  <- fread(paste0(wddata, "Mangle/groups_fmangle_L.csv"))
...
zlim_logMstar <- read_csv(paste0(wddata, "zlim_logMstar/logM_zlim_MAITE.csv"))
...
## Salvando tabelas ----
write.csv(df, paste0(wddata, "EnvQuenching/inputModel/", output_file), row.names = F)
write.csv(df_cutmass, paste0(wddata, "EnvQuenching/inputModel/", output_file_cutMass) , row.names = F)
